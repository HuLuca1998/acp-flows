# AGENTS.md · frontend/src/features/context

> 本目录的规则。**就近优先**：与上级 [`../AGENTS.md`](../AGENTS.md) 冲突时以本文件为准。

## 负责什么

**右栏（上下文面板）**。时间线事件过滤 · 权限档 · 引用文件 · 工作区状态。**只在对话主区出现**。

对应验收点 **V6**，归 `M7`——2026-08-08 里程碑重排后它还没建施工图，
验收条件见 `roadmap.md` 的「M5–M12 的验收条件」一节。
...
```bash
cd frontend && pnpm exec vitest run src/features/context
