...
| `Timeline.test.tsx` | `src/features/timeline/` | ★ 渲染行为：**文本流合并进同一气泡**（每片一个气泡的话打字过程中界面会疯狂重排）、**不同的工具调用各占一张卡片**（合并会让用户以为只动了一个文件）、未知类型照样渲染且不影响其余、载荷没有 text 时不崩<br>★★ 工具调用（真机走查补的）：同一个 `toolCallId` 的 `tool_call` + 若干 `tool_call_update` **归并成一张卡片**（不归并的话四条一模一样，用户以为动了四个文件）；卡片上显示 Agent 给的 title，没有就退到文件路径；状态取最后一次更新（停在中间态会让人以为还在跑）；**摘要不许降级覆盖但同档要覆盖**——ACP 先给泛称「Read File」再补具体的「Read README.md」，两侧各有一个负例卡着 |<br>★★ **M5 U5.3.1 角色标签**：形态照设计稿 `Claude · 需求分析师`；**角色不同就不合并**（并进去的话两个人的话挤在一个气泡里而标签只剩一个）；同角色的流式片段仍然合并；**没有角色的事件不显示这一块**（填「系统」会让用户以为有个叫系统的角色在干活）；认不出的角色照常显示消息<br>★★ **U5.3.1 R2 用户消息右对齐**：`data-align` 一端 `end` 一端 `start`——挤在同一侧的话，一屏滚下来「哪句是我说的、哪句是它说的」要逐条读文字才分得清；原话一个字不少；**不带角色标签**（给它安个「用户 · 你」会让他以为自己也是被编排的一个角色<br>★★ **U5.3.2 头像**：`CL`/`CX` 由 Runtime 名推导（对照表只管设计稿定死的那两个，其余走兜底——只有对照表的话，加一个 Runtime 忘了补一行就是个没有身份的空白方块）；没有角色的事件与用户消息都不画头像 |
...
| `ChatPage.test.tsx` | `src/features/chat/` | ★ 对话页的**用错路径**：没项目时引导去加而不是给一个点了没用的输入框；空需求不发请求（发出去后端会拒，用户看到一句莫名其妙的错误）；建工作失败**要说出来**（静默的话他不知道是没点上、在转圈、还是失败了）；建好之后才订阅事件流<br>★★ **M5 U5.1.3 R5 接着说**：已经有工作时走 `sayInWork` 而**不再建新工作**（第二句又 `startWork` 的话，新 worktree 新会话新时间线，前一句彻底不在上下文里）；按钮与提示跟着从「开始」变成「发送 / 接着说」——用户要看得出这句话会进已有的对话还是另起一个工作；工作已结束时说清楚而不是静默失败。有负例卡着（把分支短路掉，两条立刻红）<br>★★ **U10.7.3 引用文件**：加引用后发送**带 refs**（断言 `sayInWork(id, text, [path])`）、发完 chips 清空（上一句的引用不悄悄跟着下一句）；移除 chip 后发送**不带**它——「只显示不注入」是 forbidden 第一条 |
...
| `SkillPage.test.tsx`（U10.4.1 两条） | `src/features/skill/` | ★★ M10 U10.4.1 R4：**从没用过显示 0 而不是空白**——空白会让用户以为这个数字坏了，而「从没被用过」正是他判断「该不该留着这个 Skill」最需要的一条信息；用过的显示真实次数 |
| `SkillPage.test.tsx`（U10.7.2 五条） | `src/features/skill/` | ★★ M10 U10.7.2（两栏详情，用户裁定「与设计图纸完全偏离」后还的账）：R1 点一条显示 **frontmatter 块与正文**；§7.8「查看源码」在渲染与原文间切换（渲染态 `## 步骤` 变标题、井号不出现）；R3 校验没过的条目详情里说明原因**并显示他写的 frontmatter 原文**；★★ R4 正文读不到时**明说（带路径）不显示成空白**；没选中时详情栏说明怎么用，不是一片空白；★ 页头项目选择器切到项目后**真按项目拉取**（断言 `listSkills({project})`），不是换个标签装样子 |
| `MemoryPage.test.tsx`（U10.7.1 六条） | `src/features/memory/` | ★★ M10 U10.7.1（两栏详情，同上一条的裁定）：R1 点一条显示**正文全文**（标题 + 内容）；R2 「系统数据库记录」字段值**来自接口不在前端拼**（scope / created_by / 依据 / 确认人）；★★ R3 候选在详情里能「收下 / 不要」且**都不预选中**（先切候选档——「全部」不含候选，Q25）；★★ R4 正文读不到时**明说（带路径）不显示成空白**；没选中时详情栏说明怎么用；★ 范围选择器切到跨项目后按范围拉取（断言 `listMemories({scope:'*'})`） |
| `TurnSummary.test.tsx` | `src/features/timeline/` | ★★ M7 U7.4.1 R2：**没发生的那一行不显示**（断言 `[data-row="plan"]` 不在 DOM 里），而 outcome 那一行总是有；计划变了显示新版本；★ 注入那一行**把 id 列出来不只报个数**——只说「注入了 3 条」的话，用户没法判断他刚收下的那条有没有被带上；★★ 自己点的停与跑挂了**说法不同**；没排上队要说清楚话没丢 |
| `ResumeBar.test.tsx` | `src/features/resume/` | ★★ M8 U8.3.1：R1 **显示停在哪个单元不只是工作 id**（`work-03` 这个词对用户没有任何意义）；R2 一个都没有时整块不显示；★★ R4 点一条**先真的恢复再跳**——直接跳的话界面看起来能用而那一整套恢复逻辑从没跑过；★★ R3 **工作区脏时问一句不静默继续**（他手工改过那个 worktree，而恢复之后 AI 会接着往上写），确认后带 `force` 再来；恢复失败时**不跳**（跳过去他会对着一个没恢复的工作接着说话）；不自动恢复；查不了时安静地不显示。**验过负例**：拆掉脏判断，两条立刻红 |
| `MemoryCandidateCard.test.tsx` | `src/features/memory/` | ★★ M10 U10.5.1 R1：**显示正文全文不只是标题**——只给标题用户在盲选，而收下之后这条会影响后面每一轮；★★ R2 两个按钮**都不预选中**（预选「收下」他会顺手点确定，正好绕过「让他自己决定」）；R3 收下后按钮消失但**留下一条历史**（擦掉的话他回头想不起来自己收了没有）；「不要」走 reject；★★ **正文读不到时明说不显示成空白**——空白看起来像「这条记忆没内容」，他会照着这个印象直接收下，而真相是 md 文件丢了 |
| `DecisionDock.test.tsx` | `src/features/decision/` | ★★ M9 U9.2.1：**每个选项都显示影响**（没有它用户在盲选，看到两个名字而不知道选哪个会发生什么）；★★ **推荐的只标出来不预选中**——预选中的话用户会顺手点确定，而那正好绕过了「让他自己决定」这件事，判据是「没有任何按钮是选中态」；等级显示英文原值；「稍后决定」只是一句说明**不是第四个按钮**（判据是只有两个 button）；没有待决策时整块不显示 |
| `PermissionCard.test.tsx` | `src/features/permission/` | M3 U3.1.3 R1/R4：★★ 说清楚**谁要干什么、动哪个文件**（「codex 请求写入 `crates/…/events.rs` · 写入边界外」），照设计稿<br>★★ 按钮用 **Agent 给的 options 一个不漏**——自己造「允许/拒绝」两个的话，Agent 提供的第三种选项消失而用户不知道<br>★★ 点「拒绝」发的是拒绝那个 `optionId`，绝不发允许类（搞反 = 用户点拒绝而 Agent 收到允许）<br>★ 提交中禁用全部按钮（挡连点）；不越界时不说「写入边界外」（乱说会让用户对所有提示脱敏）；按工具类别说人话；认不出的类别用兜底文案不露原始码；没选项时说清楚而不是显示空卡片<br>★★ **M7 U7.2.1 边界三态**：越界标红、边界内给一句轻的确认（大多数请求都在边界内，做得太重的话用户会开始忽略这一行）、**「还没有契约」也要说出来**——只在越界时才标的话，用户分不出「边界内」与「没人定过边界」，而后者正是他最该多看一眼的时候 |
...
| `UnitAcceptance.test.tsx` | `src/features/plan/` | ★★ M8 U8.2.1：有证据标 `✓ ev-441`、没证据标 `○ 无证据`——**没证据不等于通过**，当成通过的话一个什么都没做的单元也能「全部通过」，而用户是照着这张表决定要不要点通过的；两个数**各自算**（证据多于标准时不显示成「超额通过」）；★★ **AI 转述的证据标出来**——不标的话一条转述会和一份真 diff 长得一样，而用户判断「该不该信」全靠这一点；应用采集的不标（大多数都是它，标了反而没人看）<br>★★ **U8.2.2 验收通过由用户点**：零证据时按钮点不了（那时「验收」这个动作没有内容）；点完显示 commit 且按钮消失（点第二次会撞上「没有改动」，而那个错看起来像出了问题）；拒绝的理由说清——「没有证据」与「没有改动」是两回事，用户下一步该做什么完全不同 |
| `UnitContract.test.tsx` | `src/features/plan/` | ★★ M7 完成标志 1：契约可查看全文。**边界摆在明处**——它是用户唯一的防线，藏在一次点击后面等于没有；★★ 一条允许项都没有时**明说「一个字节都不许改」**（空着的话用户以为「没限制」）；冻结由用户点，冻完按钮消失（不可逆）；空契约冻不上时说清为什么（该做的是先补一条标准）；★★ **读不到契约时也要展开**并说清楚——不展开的话用户点了按钮界面毫无变化，他不知道是没点上还是没有契约（这条先红发现的） |
| `PlanPanel.test.tsx` | `src/features/plan/` | ★★ M6 U6.3.1：**每个单元都标着由哪个角色做**（裁定三）——不标的话用户看不出「这条谁在干」，而那正是他判断该不该信这个产出的依据（实现方审查自己的产出是 INV-ATT-8 禁止的）。版本号 + 「N 子计划 · M 单元」；子计划带状态与进度（状态词**原值不翻译**，术语表硬要求）；单元带依赖与「契约未冻结」，且**冻结了的不显示那句提示**（挂在已冻结单元上会让用户以为它也没准备好）；★★ 认不出的角色**显示原始 id 不编名字**（编出来的与角色页那张表对不上）；还没规划时整块不显示且不报错 |
| `RequirementBar.test.tsx` | `src/features/work/` | ★★ M5 U5.2.1：**冻结由用户点，不由 AI 判断**——AI 说「我觉得问清楚了」和用户说「就这样」是两件事，而冻结之后这一版就进了计划与契约。版本号与条数显示；点完变「已冻结」且**按钮消失**（冻结不可逆）；★★ 还有待确认事实时说清**为什么冻不上**（那时该做的是先回答问题，不是重试），还剩几条摆在明处；★★ 还没有需求时**整条不显示且不报错**——那是新工作的常态，弹「读取需求失败」会让用户以为出了什么事;没有工作时不发请求 |
...
| `MemoryPage.test.tsx` | `src/features/memory/` | ★★ **M2 完成标志第 3 条**。这一族里最值钱的是 INV-MEM-2 在界面上的落点：候选**必须由人点一下**才生效，且调用**带上 actor**（不带的话后端拒绝——没有任何路径能让候选自己变成生效）<br>★★ 「全部」**不含候选**（Q25）：混进去的话用户会以为 AI 刚提的东西已经生效了，而它们还在等他拍板<br>★ 候选筛选只显示候选；已生效的没有审核按钮；**标出哪些会被注入**（用户看这一页就是想知道「AI 下一轮会带着哪些规矩干活」）<br>★★ 查询失败/审核失败都**说出来而不是装作成功或空态**——装作没有的话用户以为 Duet 把记忆忘光了<br>★ 没有依据时明说「（无）」 |
| `CreateProjectDialog.test.tsx` | `src/features/project/` | ★★ **M3 完成标志全部七条**。这一族守的是**先说再做**：用户交出来的是他自己的代码仓库<br>★ 列出将创建/将追加两块（后者动的是**他的文件**，所以分开），**每一步都带为什么**；打开对话框本身不创建任何东西；确认时带 `initialize:true`（不带的话他看了清单点完确认磁盘上什么都没有）<br>★★ 没看过预演就**不给创建**——那等于回到静默写；**换目录预演失败时上一次的清单要消失**，否则他点确认会对上一个目录执行而他以为在建这一个<br>★ 列出已有 Skill 并标出校验没过的原因；gh 的修复命令由后端给、前端原样显示；非 git 仓库如实说且不擅自 git init<br>★★ 浏览器形态下 `showDirectoryPicker` 只给句柄不给路径，降级成手动粘贴——装作能选然后拿假路径去请求的话，用户会在后端拿到「路径不存在」而他明明刚选过 |
| `NewWorkDialog.test.tsx` | `src/features/work/` | ★★ **M4 完成标志 1–4、7**。这一步回答用户两个问题：我的工作区现在什么样、Duet 会在哪儿干活<br>★★ 未提交改动**分已跟踪与未跟踪两个数**——合成一句「有 7 处改动」的话他没法判断要不要先提交<br>★ 三个基线选项（分支们 + 「指定 commit…」）并标出当前分支；选了哪条就传哪条（传不下去的话他选了 develop 而工作还是从当前分支开的）；「指定 commit」没填就不给确认<br>★★ 三种探测失败**说清是哪一种**（rebase 中途 / 还没提交过 / 不是仓库）——要做的事完全不同；认不出的错误**原样显示**而不是一句「出错了」<br>★★ 明说「你的工作区不受影响」与「失败进 initializing_failed 不退回原目录」——用户最怕的就是它在自己正改的分支上乱来<br>★ 「将创建」只说形态**不编一个具体的分支号**：编出来的与实际拿到的对不上，比不说更糟 |
| `WorkspacePanel.test.tsx` | `src/features/context/` | ★★ **M4 完成标志第 6 条**。这一栏回答「AI 到底动了什么」。2026-08-09 之前右栏是「时间线过滤器 + 引用」两块占位，而设计稿里过滤器属于**计划面板那一侧**<br>★★ 未提交改动**逐个文件带增删行数**（`+64 −12`）+ 合计——只说「改了 2 个文件」判断不出改动有多大<br>★★ 没有基线时**不显示「领先 0」**：那会让用户以为 AI 什么都没干，而实际是我们不知道起点<br>★ 本次工作的 commit 带 sha / 相对时间 / 「仅本地」；还没切 worktree 时明说而不是空白<br>★★ 读不到状态要**说出来**，不装作「什么都没改」——装作没改的话用户会以为 AI 一事无成 |
