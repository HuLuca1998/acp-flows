...
| `UnitAcceptance.test.tsx` | `src/features/plan/` | ★★ M8 U8.2.1：有证据标 `✓ ev-441`、没证据标 `○ 无证据`——**没证据不等于通过**，当成通过的话一个什么都没做的单元也能「全部通过」，而用户是照着这张表决定要不要点通过的；两个数**各自算**（证据多于标准时不显示成「超额通过」）；★★ **AI 转述的证据标出来**——不标的话一条转述会和一份真 diff 长得一样，而用户判断「该不该信」全靠这一点；应用采集的不标（大多数都是它，标了反而没人看） |
