...
| `SkillPage.test.tsx`（U10.7.2 五条） | `src/features/skill/` | ★★ M10 U10.7.2（两栏详情，用户裁定「与设计图纸完全偏离」后还的账）：R1 点一条显示 **frontmatter 块与正文**；§7.8「查看源码」在渲染与原文间切换（渲染态 `## 步骤` 变标题、井号不出现）；R3 校验没过的条目详情里说明原因**并显示他写的 frontmatter 原文**；★★ R4 正文读不到时**明说（带路径）不显示成空白**；没选中时详情栏说明怎么用，不是一片空白 |
| `MemoryPage.test.tsx`（U10.7.1 五条） | `src/features/memory/` | ★★ M10 U10.7.1（两栏详情，同上一条的裁定）：R1 点一条显示**正文全文**（标题 + 内容）；R2 「系统数据库记录」字段值**来自接口不在前端拼**（scope / created_by / 依据 / 确认人）；★★ R3 候选在详情里能「收下 / 不要」且**都不预选中**（先切候选档——「全部」不含候选，Q25）；★★ R4 正文读不到时**明说（带路径）不显示成空白**；没选中时详情栏说明怎么用 |
