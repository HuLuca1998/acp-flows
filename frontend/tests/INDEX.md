...
| `ResumeBar.test.tsx` | `src/features/resume/` | ★★ M8 U8.3.1 R1：**显示停在哪个单元不只是工作 id**——只有 id 的话用户看到「work-03 · work-05」，那两个词对他没有任何意义；R2 一个都没有时**整块不显示**（那是绝大多数人每次打开的状态）；R4 点一条把 work_id 交出去；★★ **不自动恢复**——自动跳进去的话，他想开个新工作却落在昨天那个半截的现场里；查不了时**安静地不显示**，不在首屏摆红字让他以为应用坏了 |
