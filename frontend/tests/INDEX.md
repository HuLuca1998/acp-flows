...
| `Timeline.test.tsx` | `src/features/timeline/` | ★ 渲染行为：**文本流合并进同一气泡**（每片一个气泡的话打字过程中界面会疯狂重排）、**工具调用不合并**（两次就是两次，合并会让用户以为只动了一个文件）、未知类型照样渲染且不影响其余、载荷没有 text 时不崩 |
| `use-event-stream.test.ts` | `src/features/timeline/` | ★ SSE 订阅：**按 `work_id` 过滤**（事件流是全局的，不过滤的话开着 A 工作却看到 B 的 AI 在说话）；**按 seq 去重**（重连时服务端会重发边界那条——我们宁可重复也不愿缺失）；**卸载与切换工作时关连接**（不关的话每切一次泄漏一条，后端每条事件要往所有订阅者投一遍）；坏消息跳过不断流 |
