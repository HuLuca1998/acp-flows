...
func FixedClock(t time.Time) *fixedClock { return &fixedClock{t: t} }
...
func SeqIDGen() *seqIDGen { return &seqIDGen{seq: map[string]int{}} }

func (g *seqIDGen) NextID(prefix string) string {
	g.mu.Lock()
	defer g.mu.Unlock()
	g.seq[prefix]++
...
func (g *seqIDGen) NextULID() string {
	g.mu.Lock()
	defer g.mu.Unlock()
	g.ulid++
	// 定宽零填充，保证字典序 == 生成顺序。
