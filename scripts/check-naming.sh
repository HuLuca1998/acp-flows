...
say() { fail=1; echo "✗ $1"; shift; printf '    %s\n' "$@"; echo; }
...
# ── 3. Go：禁止 Get 前缀的访问器 ──────────────────────────────
getters=$(src -name '*.go' ! -name '*_test.go' \
  -exec grep -HnE '^func +\([^)]+\) +Get[A-Z]' {} + 2>/dev/null | sed 's|^\./||' || true)
if [[ -n $getters ]]; then say "Go 访问器禁止 Get 前缀，直接用字段名（见 coding-standards §3.3）" $getters; fi

# ── 4. TS：禁止 enum ─────────────────────────────────────────
enums=$(src \( -name '*.ts' -o -name '*.tsx' \) \
  -exec grep -HnE '^ *(export +)?enum +' {} + 2>/dev/null | sed 's|^\./||' || true)
...
if [[ -d backend ]]; then
  leak=$(grep -rn 'tests/testutil' backend --include='*.go' 2>/dev/null \
