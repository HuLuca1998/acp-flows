...
> 决策依据见 [`adr/`](../adr/)。本文描述**目标架构**；当前实现进度见 `roadmap.md`。
...
**自动更新 + 本地数据库 = 一类特有风险**：用户不是 DBA、更新是自动的、
而且**用户可能装回旧版**。所以 `duetd` 的启动是有严格顺序的：

```
duetd 启动
  │
  ├─① 打开 DB（WAL / foreign_keys / busy_timeout）
  ├─② 降级检查   DB 的 schema 版本 > 本二进制内嵌的最高版本？ → 拒绝启动，非零退出
  ├─③ 备份       仅当有待应用迁移时；VACUUM INTO，失败即中止
  ├─④ 迁移       单事务，按版本顺序；失败整体回滚并退出
  ├─⑤ seed       幂等种子数据，ON CONFLICT DO NOTHING
  └─⑥ listen     ★ 到这一步才开始接受 HTTP 请求
```
...
| 约束 | 为什么 |
|---|---|
| **②：旧程序遇到更高 schema 拒绝启动** | 旧代码读新 schema 不会崩，会**静默产生错误结果**——比崩溃危险得多 |
| **③：备份用 `VACUUM INTO`，不是 `cp`** | WAL 下数据可能还在 `-wal` 里，实测拷主库文件得到的副本**连表都没有** |
| **⑥：迁移完成才 listen** | 壳拉起 sidecar 后前端立刻发请求。迁移在请求路径上跑 = 超时或读到半个 schema |

**一键初始化**在正常路径上是不可见的：数据目录不存在时自动建、
从 0 跑完迁移、seed，用户第一次打开就能用，不该看到「正在初始化」的界面。
手动重置（设置页 / `duetd --reset`）先备份再重建，**二次确认**。

完整决策与取舍见 [`../adr/0008`](../adr/0008-schema-migration-and-init.md)，
迁移文件的写法见 [`../rules/database.md`](../rules/database.md) §5。
