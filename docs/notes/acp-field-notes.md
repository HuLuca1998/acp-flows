...
### 2026-08-09 复验：两道防线是**配合**的，不是二选一

用 `cmd/restrictprobe` 走生产代码那条路（`set_config_option` 收权 → 发一句
「建个文件」→ **看磁盘**）复验了两端。判据在磁盘上，不在协议帧上——
「我们发了收权请求」这件事本身证明不了任何东西。

| 端 | 只读档名 | 它做了什么 | 磁盘 |
|---|---|---|---|
| claude | `plan` | 自己说「我现在处于 plan 模式，**不能直接写文件**——这个限制优先于「直接做」的指令」 | ❌ 没建 |
| codex | `read-only` | **仍然去试**：`Attempting patch application despite read-only`，然后那次写**触发了审批**，被客户端拒掉，`stopReason=cancelled` | ❌ 没建 |

★★ codex 那条把两道防线的关系说透了：

- **档位的作用不是「让它不想写」**，而是**让写操作变成需要审批的**
- 客户端裁决这才有了落点——在默认 `agent` 档下它压根收不到那次请求

所以 ①档位 与 客户端裁决 是**串联**的：档位把动作暴露到审批面上，
裁决再决定放不放。**只做任何一道都拦不住**——这正是上面那张表里
「`agent` 档 + 全拒 = 文件建了」的原因。
