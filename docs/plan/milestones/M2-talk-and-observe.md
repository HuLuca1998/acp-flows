...
### ✓ U2.4.1 · 新建工作：worktree、会话、时间线接线  ·  `429f329` + `6aa70bb` + `252cc76`

> **2026-08-08 已完成**，**真机跑通**：浏览器输入需求 → 拉起真 `claude-agent-acp`
> → 工具调用与回答逐条出现在时间线 → `turn_end`。用户仓库一个字节没动，
> 一轮跑完没有孤儿进程（`ps` 查过 pid）。
>
> **R5（真 Agent 集成测试）换了个做法**：没有用 `//go:build integration`，
> 而是在 `runner_test.go` 里用**真进程**——`t.TempDir()` 里的 shell 脚本按 ACP
> 规范说话。它验的正是「进程怎么拉起来、拉不起来时用户看到什么、跑完有没有
> 留孤儿」，而且不依赖开发机上装没装 Agent、登没登录。真 Agent 的差异行为
> 仍归 `U2.2.3` 的 R4，那笔欠账留着。
>
> **真机走查抓到四个单测测不出的 bug**，见 `docs/plan/roadmap.md` 的表。
> 最贵的一条教训：替身要换在**真实现真正走的那一层**——把整个 `EventSource`
> 换成假的，等于把出问题的那部分一起替换掉了，401 单测再多也测不出。
...
### ✓ U2.3.2 · 时间线渲染与过滤  ·  `f38f316` + `dd9f2ea`（接线）
...
> **2026-08-08 已接线**（`dd9f2ea`，随 `U2.4.1` 一起）：对话页调
> `POST /v1/works` 建工作，`useEventStream` 连 SSE 并按 `work_id` 过滤，
> 事件喂给 `Timeline`。真机走查过：浏览器里输入需求 → 后端建出工作 →
> SSE 连上 → 事件到达。
