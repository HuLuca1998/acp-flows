...
> 实操（2026-08-08）：**⚠ 还没验**。`v0.0.1` 已发布（四类产物齐、三方签名过），
> 但**没有人在干净机器上装过一次**。归 `U1.2.3`——它需要真的下载、拖进
> 「应用程序」、双击。本机构建过的产物不算数：那绕过了 Gatekeeper 与
> 首次启动的签名检查，而那两处恰恰是最容易出问题的。

2. 我发一个新版本 → 他进「设置 → 应用更新」→ 看到「发现新版本」→ 点「⟳ 一键更新并重启」→ **应用重启后版本号变了**

> 实操（2026-08-08）：**⚠ 还没验**。要先有人装上 `v0.0.1`，再发 `v0.0.2`
> 才能走通。这条同时是 `U4.1.2` 的 R4（更新前暂停 → 更新后恢复），
> 两者一起验。

3. 进「设置 → 环境检测」→ **看得出 Claude / Codex 装没装、登没登录**

> 实操（2026-08-08）：设置 → ACP Runtime 检测，截图确认——
> `claude 0.63.0` 已安装 · 已登录、`codex 1.1.7` 已安装 · 已登录，
> 各自带可执行文件的完整路径。左栏底部也同步显示两条 `● 就绪`。
