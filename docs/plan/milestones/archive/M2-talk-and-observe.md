...
> 实操：**✗ 做不到（2026-08-08 用户当场发现）**。左栏「项目」「最近」两块
> 是骨架占位，`/v1/projects` 明明有数据；「创建项目」按钮是 `disabled` 的死按钮。
> 之前验第 2、3 条时是**自己用 curl 建的项目**——那不是用户会做的事。
> 归 `U5.1.1`。

2. 「新建对话」输入一句需求 → AI **追问细节**（不是直接开写），文字**边说边显示**

> 实操：浏览器里输入「读一下 README.md 说说写了什么」→ 点开始 →
> 拉起真 `claude-agent-acp` → 时间线出现 `工具调用 / Read README.md` →
> AI 的回答逐字显示 → `本轮结束`。★ 但入口是主区那个输入框，
> **不是设计稿里的「新建对话」**（那个在左栏项目下面，归 `U5.1.1`）。

3. 中间区域像一条时间线：AI 说了什么、读了哪个文件、跑了什么命令，都看得见

> 实操：同上那一轮，时间线依次是 `状态变动` → `工具调用 Read README.md` →
> `消息`（AI 的回答）→ `本轮结束`。刷新页面后历史还在。
