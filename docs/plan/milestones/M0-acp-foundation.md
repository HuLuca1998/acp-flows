...
> **读法**：本文 ~8.3k token，**不要整篇读**。你只需要你那个单元那一节。
>
> | 你要找 | 怎么定位 |
> |---|---|
> | 我该做哪个单元 | 读下面的「当前进度」块，就三行 |
> | 某个单元的契约 | `grep -n 'U0.4.1' docs/plan/milestones/M0-acp-foundation.md` → 从那行读 30 行 |
> | 子计划的依赖关系 | `grep -n '子计划 DAG' …` |
> | 某条裁定的来龙去脉 | `grep -n '裁定' …` |
> | M0 整体怎么算完 | `grep -n 'M0 验收' …` |
...
| | |
|---|---|
| `goal` | Fake 能主动发 `session/request_permission` 并等应答，能按声明表现出不同的**协议行为** |
| `allowed_changes` | `backend/internal/acp/fake/**` |
| `forbidden_changes` | 同上；**不得 import `capability`**（见下面的裁定） |
| `stop_conditions` | — |
...
| R3 | 声明的能力**表现为真实协议行为**，不是打假返回值 | 声明不支持会话模式 → `session/new` 响应里**真的没有** `modes` 字段；声明不支持 `loadSession` → `initialize` 的 `agentCapabilities` 里真的是 `false` |
| R4 | `optionId` **原样回传**，不按 `kind` 匹配 | Fake 发一组 `optionId` 与 `kind` 语义不一致的选项，断言收到的应答里是原样的 `optionId` |
| R5 | 权限请求**没有超时** | 断言等待 2s 后 turn 仍未结束——挂几小时等用户回来是产品语义，不是故障（§7.3） |

> **R3 的裁定（2026-08-07）**：原文写的是「可声明 12 项探针的任意通过组合，
> 断言能力矩阵与声明一致」，依据 `acp-integration.md` §12.6 的
> `Capability{Probes map[capability.ProbeID]capability.Outcome}`。
> **这与 §3.3 硬规则 2 冲突**——`fake` 只允许 import `protocol`，而 `capability`
> 是被测代码，Fake 依赖它就没资格当参照物了。
>
> 12 项探针是 Duet 自己定义的概念（§8.1 标 `[DUET]`），不属于协议，也不该塞进 `protocol`。
> 所以 Fake 用**自己的能力开关类型**（像 `Clock` 那样重复定义一个小接口），
> 只负责「表现出对应的协议行为」。§12.6 本身也是这么要求的：
> 「假的能力声明必须表现为真的协议行为，否则测的还是我们自己的探针代码」。
>
> **两边的对齐留给 `U0.8.2` 的跨实现契约测试** —— 那才是验证
> 「探针读到的能力矩阵 == Fake 声明的能力」的正确位置。
>
> **原 R4「Fake 自己跑一致性契约测试」已移交 `U0.8.2`**：它依赖的东西隔着
> 两个子计划（`capability` 与 `adapter` 都还不存在），留在这里等于一条永远无法验收的标准。
> 移交后本单元腾出的编号给了 `optionId` 原样回传——那是 §7.1 明确点名的陷阱。
...
| R4 | **探针读出来的能力矩阵 == Fake 声明的能力**（从 U0.4.2 移交） | 让 Fake 声明「不支持会话模式」，跑真实探针，断言矩阵里对应项为不通过 |

> **R4 是从 `U0.4.2` 移交过来的**（2026-08-07）。它原本写在 Fake 那个单元里，
> 但 `fake` 只允许 import `protocol`，够不着 `capability` —— 留在那儿等于
> 一条永远无法验收的标准。这里才是它的正确位置：`capability` 与两个 adapter
> 都已存在，可以真的跑一遍探针去读 Fake。
>
> **这条是关键**：Fake 若与真实 adapter 行为不一致，所有依赖它的上层测试都是假的。
> 而「不一致」最隐蔽的形态就是——Fake 声称自己不支持某能力，探针却仍然探到支持。
