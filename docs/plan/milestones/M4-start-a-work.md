...
1. 点项目下的「＋ 新建对话」→ **弹出「新建工作」对话框**
   ...
   > 实操（2026-08-09）：浏览器点 `duet-demo2` 下的「新建对话」→ 弹层出来了，
   > 标题与副标题照设计稿（「选择本机 Git 仓库 → 基线分支 → 独立 worktree」）。

2. 看到「已检测到 `.git` · 无未提交改动」，或**如实报告有几个文件改了**

   > 实操：仓库里造一个已跟踪改动 + 一个未跟踪文件 →
   > 显示「已跟踪文件改了 1 处 · 另有 1 个未跟踪文件」。
   > ★ **两个数分开报**：合成一句「有 2 处改动」的话，用户没法判断
   > 要不要先提交——新建的临时文件和改过的代码是两件事。

3. 能选基线分支：`main` / `develop` / 「指定 commit…」

   > 实操：三个选项都在，当前分支 `main` 默认选中并标「（当前）」；
   > 选「指定 commit…」会出输入框，没填之前确认按钮是禁用的。

4. 看到将创建的分支名、worktree 路径、基线 commit 短号

   > 实操：显示「一条以 duet/ 开头的分支」「~/.acpflows/worktrees/ 下的一个目录」
   > 「基线：main @ e6748ca」。
   > ★ **不编一个具体的 `duet/work-09`**：编出来的与实际拿到的号对不上，
   > 比不说更糟。

5. 点「创建 worktree 并开始」→ 工作出现在项目下，**磁盘上真的多了 worktree**

   > 实操：`AddWorktree` 真机建出 `/tmp/duet-wt/work-demo`，
   > 分支 `duet/work-demo`，基线 `e6748ca`。

6. 右栏出现「工作区」：分支名、领先几个 commit、未提交改动、本次工作的 commit 列表

   > 实操：在 worktree 里造一条提交 + 一处未提交改动 → 探测得到
   > `Ahead=1`、`new-feature.txt +1 −0`、`0fcf25c unit-001: 加了个功能`。
   > ★ 右栏原本是「时间线过滤器 + 引用」两块占位——**设计稿里那两块
   > 根本不在右栏**（过滤器属于计划面板那一侧）。落位记进了 `PARITY.md`。

7. 创建失败时工作进入 `initializing_failed`，**绝不退回原目录执行**

   > 实操：`Start` 的失败分支把工作推到 `initializing_failed` 并如实报错；
   > `TestAddWorktree_RefusesUnknownBase` 断言基线解析失败时**目录都不建**。
   > ★ 代码里没有任何「worktree 失败则用项目根目录」的回落分支。
