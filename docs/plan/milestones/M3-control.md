...
> **2026-08-08 已完成，M3 到此全部收尾。真机验过**：AI 正在流式写长文时点
> 「停下」→ 状态变 `● paused`、按钮消失、**时间线保留全部历史**、
> 末尾多出 `本轮结束 → 状态变动 → 检查点`，没有孤儿进程。
>
> **`stop_conditions` 的那一条（设计稿里找不到取消态）判定为不成立**：
> 它是已有语汇的组合，不是新发明——状态词照设计稿的 `executing · 3/7`
> （等宽英文原值 + 状态圆点），「停下」照权限卡片的「拒绝」（灰边框次要按钮）。
> 铁律 3 防的是**临时发明样式**。判断依据写进了 `features/work/AGENTS.md`。
>
> **走查抓到两个连锁 bug**（单测都测不出）：
>
> | 撞到的 | 根因 |
> |---|---|
> | 点停之后界面说「失败」 | 那一轮因 `stopReason=cancelled` 返回错误，被「AI 跑挂了」那条路径抢先推到 failed |
> | 改完仍是 `[… paused failed]` | 取消标记在 `Cancel` 返回时就清了，而后台那一轮还没结束 |
> | `clarifying → paused` 被状态机拒 | `CanCancel` 说能停，迁移表里却没这条出边 |
>
> 第三个已变成检查：`TestCanCancel_MatchesTransitionTable` 逐个核
> 「能取消」与「迁得到 paused」是否一致。
