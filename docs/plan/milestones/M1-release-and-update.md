...
> | S1.7b ★ | **更新时的数据库安全**：降级保护 · VACUUM INTO 备份 · 一键初始化 |
> | S1.8 | 更新界面 · 一键更新重启 · 恢复 |
...
S1.2 版本推导  S1.3 Tauri 壳  S1.4 前端骨架  S1.6 系统端点  S1.9 Runtime 更新
   ...
   └──────┬───────┘              │              │
          ▼                      │              ▼
  S1.5 构建·签名·发布             │        S1.7 prepare + S1.7b DB 安全
...
### ○ U1.7.3 · Checkpoint 最小实现（`adr/0006` Q35）

| | |
|---|---|
| `goal` | `Checkpoint` 聚合的最小可用版：**commit hash + Work 状态快照 + 事件游标**三样。证据关联留给 M2 |
| `allowed_changes` | `backend/internal/domain/model/checkpoint.go` 及其测试 · `backend/internal/constant/checkpoint.go` · `backend/internal/store/entity/checkpoint.go` · `backend/internal/store/mapper/checkpoint.go` · `backend/internal/store/checkpoint_repo.go` · `backend/internal/store/migration/NNNN_create_checkpoints.sql` · `backend/internal/app/port/checkpoint.go` |
| `forbidden_changes` | **不做证据关联**（`evidence_refs` 那一组字段属于 M2 S2.7，本单元连列都不加）；`domain` 不得 import 任何内部包，不得出现 `context.Context` / `time.Now()`；不提供任何删除 Checkpoint 的路径 |
| `stop_conditions` | 需要在 Checkpoint 上挂证据才能让 `prepare` 闭环 —— 说明最小版切错了，**停下来上报**，不要顺手把 M2 的范围拉进来 |

**验收标准**

| # | 标准 | 断言 |
|---|---|---|
| R1 | 三样缺一即创建失败 | 三个用例（缺 commit hash / 缺状态快照 / 缺事件游标）各断言对应错误，错误信息含缺失字段名 |
| R2 | ★ `commit_hash` 必须是 40 位十六进制，短 hash 与分支名都拒绝 | 表驱动：40 位 ✓ · 7 位短 hash ✗ · `main` ✗ · 空 ✗ |
| R3 | ★ 状态快照存的是**创建时**的 Work 状态，之后 Work 变了它不跟着变 | 落检查点 → 改 Work 状态 → 断言检查点里的快照仍是旧值（不是引用） |
| R4 | 事件游标单调：同一 Work 的后一个检查点游标 ≥ 前一个 | 造两个检查点，断言 `seq` 不回退；构造回退用例断言报错 |
| R5 | Checkpoint 没有物理删除路径（与 INV-MEM-6 同源） | 反射断言导出方法名集合里不含 `Delete` / `Remove` / `Purge` 前缀 |
| R6 | 存进去再取出来三样字段不丢 | 临时 SQLite 往返，逐字段断言相等 |
| R7 | 覆盖率 ≥ 90%（`domain` 部分） | `make cover` |

> **为什么最小版排在 U1.7.2 之前无法闭环、之后又太晚**：`prepare` 要落检查点，
> 而完整 Checkpoint（含证据关联）在 M2。M1 内部若不切一个最小版，
> `prepare` 就只能落一个假的检查点——那比不落更糟，
> 因为 `GET /system/resume` 会把它列出来，用户点了恢复才发现恢复不了。
>
> R3 是最容易写错的一条：**快照不是引用**。存了引用的话，
> 更新期间 Work 状态一变，检查点记录的就不是它被创建时的现场了。

---

## S1.7b · 更新时的数据库安全 ★

**阶段交付物**：新版本启动时数据库被**准确迁移**；旧版本启动时**拒绝**碰新 schema；
迁移前有可用备份；全新安装零操作可用。

> 裁定见 [`../../adr/0008`](../../adr/0008-schema-migration-and-init.md)。
> **自动更新 + 本地数据库是一类特有风险**：用户不是 DBA、更新是自动的、
> 而且用户可能装回旧版。本子计划就是把这三件事挡住。
>
> 启动序列（顺序不可调）：
> `打开 DB → 降级检查 → 备份 → 迁移 → seed → listen`

### ○ U1.7b.1 · 降级保护：旧程序遇到更高 schema 拒绝启动

| | |
|---|---|
| `goal` | `MAX(schema_migrations.version) > 二进制内嵌的最高迁移号` 时，`duetd` 非零退出并给出可自解释的错误 |
| `allowed_changes` | `backend/internal/store/migration/migration.go` · `backend/internal/store/migration/migration_test.go` |
| `forbidden_changes` | 不改任何已有 `.sql`（checksum 会炸）；不改 `store.Open` 的签名；不加降级迁移路径 |
| `stop_conditions` | 若发现有合理场景需要「旧版本只读打开新库」——**停下来问人**，不要自己加开关 |

**验收标准**

| # | 标准 | 断言 |
|---|---|---|
| R1 | 库里最高版本 == 内嵌最高版本 → 正常启动 | 断言 `Run` 返回 nil |
| R2 | ★ 库里最高版本 **>** 内嵌最高版本 → 返回错误，**且不执行任何 SQL** | 手工插入 `schema_migrations(version=9999)`；断言返回 error，且 spy 记录的 Exec 次数 == 0 |
| R3 | 错误信息含两个版本号与备份目录路径 | 断言 message 里同时出现 `9999`、内嵌版本号、`backup` |
| R4 | 库里版本 < 内嵌版本 → 正常升级 | 断言迁移被应用、`schema_migrations` 补齐 |

> **为什么必须是「拒绝」而不是「警告后继续」**：旧代码读新 schema 不会崩溃，
> 它会读到少了列的表、多了成员的枚举，然后**静默产生错误结果**。
> 那比崩溃危险得多——没有任何人会发现。

### ○ U1.7b.2 · 迁移前备份：`VACUUM INTO` ★

| | |
|---|---|
| `goal` | 有待应用迁移时，先用 `VACUUM INTO` 做一致性备份；备份失败则**中止迁移** |
| `allowed_changes` | `backend/internal/store/backup.go`（新）· `backend/internal/store/backup_test.go`（新）· `migration.Run` 的调用点 |
| `forbidden_changes` | **绝不用 `os.ReadFile`/`io.Copy` 拷贝 db 文件**（见下）；不自动删用户备份之外的任何文件 |
| `stop_conditions` | 保留策略的份数若需要超过 3 份，先问人——那是磁盘占用的取舍 |

**验收标准**

| # | 标准 | 断言 |
|---|---|---|
| R1 | ★ WAL 里有**未 checkpoint** 的数据时，备份内容**完整** | 写入 N 行后不 checkpoint，直接备份；打开备份断言 `COUNT(*) == N` |
| R2 | ★ 对照组：裸文件拷贝**做不到** R1 | 同样条件下 `os.ReadFile` 复制，断言副本查询报错或行数 < N。**这条固定住"为什么不能用 cp"** |
| R3 | 无待应用迁移时**不产生备份文件** | 二次启动后断言 backup 目录文件数不变 |
| R4 | 备份失败（目录不可写）→ 迁移**不开始** | 断言返回 error 且 `schema_migrations` 未变化 |
| R5 | 目标文件已存在时不覆盖 | 文件名带时间戳；断言两次备份产生两个文件 |
| R6 | 保留策略：超过 3 份时删最旧的 | 造 5 份，断言剩 3 份且留下的是最新的 |
| R7 | 全程不碰 `~/.acpflows` | 铁律 6 守卫断言 |

> **R2 是本单元的核心。** 实测过：WAL 下 `cp duet.db` 得到的副本
> 连表都没有（`no such table`）——数据还在 `-wal` 里。
> 见 [`../../notes/pitfalls.md`](../../notes/pitfalls.md) P-18。

### ○ U1.7b.3 · 种子数据与一键初始化

| | |
|---|---|
| `goal` | 全新安装零操作可用：空目录 → 建目录 → 迁移到最新 → seed 推荐角色集。`duetd --reset` 先备份再重建 |
| `allowed_changes` | `backend/internal/store/seed/**`（新）· `backend/cmd/duetd/main.go` · 对应测试 |
| `forbidden_changes` | **种子数据绝不写进 `.sql` 迁移**（checksum 锁死后就改不了了）；seed 不得覆盖用户已修改的记录 |
| `stop_conditions` | 推荐角色集的具体内容若在设计稿里找不到，按 `frontend-guide.md` §16 登记缺口，不要自己编一套 |

**验收标准**

| # | 标准 | 断言 |
|---|---|---|
| R1 | 空数据目录启动 → 可用状态，零用户操作 | 断言目录被创建、迁移到最新、seed 记录存在 |
| R2 | ★ seed 幂等：跑两次结果一样 | 断言两次后行数与内容一致 |
| R3 | ★ **用户改过的种子记录不被覆盖** | 改一条 seed 记录后重启，断言修改仍在 |
| R4 | `--reset` 先备份再重建 | 断言备份文件存在且含旧数据，新库是干净的 |
| R5 | 迁移未完成前**不 listen** | 用慢迁移替身；断言迁移返回前端口未监听 |

> R5 的理由：壳拉起 sidecar 后前端立刻发请求。
> 迁移在请求路径上跑 = 第一个请求超时，或读到半个 schema。
...
### ○ U1.8.4 · 启动恢复未完成的工作（`adr/0006` Q40，原 `U4.7.1`）
...
| `stop_conditions` | 恢复需要重建已经不存在的 ACP 会话而 `session/load` 失败 —— 降级路径必须显式标记，**不许假装连续**；U1.7.3（Checkpoint 最小版）未合入 → 停 |
...
| R7 | 测试不碰用户真实仓库 | 用临时 git 夹具仓库；铁律 6 守卫断言 |

> R4 直接对应前一个项目的 **H-4**：界面从 DB 渲染出了历史，
> 但 ACP 会话早就不在了——**用户以为 Agent 记得，其实它什么都不知道**。
> 这不是显示问题，是会让人做出错误决定的问题。
