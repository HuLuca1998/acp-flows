...
| 顺序 | 里程碑 | 第一个该做的单元 | 前置 |
|---|---|---|---|
| 1 | **M0** 地基 | **`U0.2.3` `protocol` 线格式包** ★ | 无，可立即开工 |
...
```
U0.2.3 → U0.4.1 → U0.4.2   protocol → Fake Runtime  ← 关键路径，优先派人
U0.2.2                     子进程生命周期            ← 与 Fake 无交集
U0.9.2                     PlanVersion/契约冻结      ← 纯 domain，与协议层零交集
```

> **`U0.2.3` 是 2026-08-07 补登记的。** 原先写「`U0.4.1` 可立即开工」是错的：
> `fake` 只允许 import `protocol`，而那个包当时并不存在。
